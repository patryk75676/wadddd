...
:: Use compiled agent.exe if present (PyArmor+Nuitka build), else fall back to Python script
if exist "%~dp0agent.exe" (
    copy /y "%~dp0agent.exe" "%INSTALL_DIR%\" >nul
    set AGENT_BIN="%INSTALL_DIR%\agent.exe"
    set NEEDS_PYTHON=0
) else (
    copy /y agent.py              "%INSTALL_DIR%\" >nul
    copy /y wol_setup.py          "%INSTALL_DIR%\" >nul
    copy /y hardware_detect.py    "%INSTALL_DIR%\" >nul
    set AGENT_BIN=pythonw.exe "%INSTALL_DIR%\agent.py"
    set NEEDS_PYTHON=1
)
...
:: Create service — compiled exe or python script
sc create %SVC_NAME% binPath= "%AGENT_BIN%" start= auto DisplayName= "%SVC_DISPLAY%"
